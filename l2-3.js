(function () {
  ...
  function unlocked() {
    return Boolean(window.EXCEL_LAB_DEV?.enabled) || Boolean(profile?.progress?.["l2-2"]?.completed);
  }
  function progress() {
  ...
  function save(next) {
    ...
    // A revoked L2.3 completion must also revoke the next lesson's completion.
    if (!next.completed && profile.progress[ID]?.completed && profile.progress["l2-4"]) {
      profile.progress["l2-4"].completed = false;
    }
  ...
  function render() {
    ...
    $("#l23-content").hidden = !open;
    $("#l23-access").hidden = open;
    $("#l23-access-message").textContent = profile
      ? "Schließe L2.2 mit allen eigenen Checks und der Lehrkraftbestätigung ab. Danach kannst du hier weiterlernen."
      : "Lege auf der Startseite dein Lernprofil an und schließe L2.2 ab.";
    ...
    $("#l23-mastery-form").querySelectorAll("input, button").forEach((element) => {
      element.disabled = !open || current.masteryPassed || Boolean(window.EXCEL_LAB_DEV?.enabled);
    });
    $("#l23-mastery-form").querySelectorAll(".mastery-question, button[type='submit']").forEach((element) => {
      element.hidden = current.masteryPassed;
    });
    const masteryStatus = $("#l23-mastery-status");
    if (current.masteryPassed) {
      masteryStatus.textContent = "Verständnis-Check bestanden. Jetzt noch beide Excel-Dateien prüfen und mit der Lehrkraft besprechen.";
    ...
    button.textContent = current.completed ? "✓ L2.3 wieder öffnen" : "L2.3 abschließen";
    ...
    $("#page-completion-note").textContent = current.completed
      ? "100 Punkte wurden gutgeschrieben. Beim Wiederöffnen wird L2.4 erneut gesperrt; ein dortiger Abschluss wird zurückgenommen."
  ...
  $("#l23-mastery-form").addEventListener("submit", (event) => {
    ...
    if (!allCorrect) {
      $("#l23-mastery-status").textContent = "Noch nicht bestanden. Lies die Hinweise und versuche es erneut.";
    ...
    if (save(next)) {
      render();
      toast("Verständnis-Check bestanden. Zeige nun beide Excel-Dateien der Lehrkraft.");
    }
  });
  ...
  document.addEventListener("change", (event) => {
    ...
    render();
    if (saved && revoked) toast("Abschluss zurückgenommen. L2.4 ist wieder gesperrt.");
  });
  $("#page-complete-button").addEventListener("click", () => {
    ...
    if (!next.completed && !next.masteryPassed) {
      $("#l23-mastery-section").open = true;
      $("#l23-mastery-section").scrollIntoView({ block: "start", behavior: "smooth" });
    ...
    const saved = save(next);
    render();
    if (saved) toast(next.completed ? "L2.3 abgeschlossen: 100 Punkte. L2.4 ist freigeschaltet." : "L2.3 ist wieder offen. L2.4 ist wieder gesperrt.");
  });
