(function () {
  ...
  function unlocked() {
    return Boolean(window.EXCEL_LAB_DEV?.enabled) || Boolean(profile?.progress?.["l1-5"]?.completed);
  }
  function progress() {
  ...
  function save(next) {
    ...
    // A revoked L1.6 completion must also revoke the next lesson's completion.
  ...
  function render() {
    ...
    $("#l16-content").hidden = !open;
    $("#l16-access").hidden = open;
    $("#l16-access-message").textContent = profile
      ? "Schließe L1.5 mit allen eigenen Checks und der Lehrkraftbestätigung ab. Danach kannst du hier weiterlernen."
      : "Lege auf der Startseite dein Lernprofil an und schließe L1.5 ab.";
    ...
    button.textContent = current.completed ? "✓ L1.6 wieder öffnen" : "L1.6 abschließen";
    ...
    $("#page-completion-note").textContent = current.completed
      ? "100 Punkte wurden gutgeschrieben. Beim Wiederöffnen wird L2.1 erneut gesperrt; ein dortiger Abschluss wird zurückgenommen."
