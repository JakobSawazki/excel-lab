(function () {
  ...
  function unlocked() { return Boolean(profile?.progress?.["l1-1"]?.completed); }
  function progress() {
  ...
  function render() {
    ...
    checks.forEach((el, i) => { el.checked = p.checks[i]; el.disabled = !open; });
    $("#page-teacher-check").checked = p.teacherChecked; $("#page-teacher-check").disabled = !open;
    const button = $("#page-complete-button"); button.disabled = !open;
    ...
    const next = $("#next-lesson-link"), ready = open && p.completed;
  ...
  $("#next-lesson-link").addEventListener("click", (event) => {
    refresh(); if (!unlocked() || !progress().completed) { event.preventDefault(); render(); toast("Schließe zuerst L1.2 ab."); }
  });
