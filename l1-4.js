(function () {
  ...
  function unlocked() { return Boolean(window.EXCEL_LAB_DEV?.enabled) || Boolean(profile?.progress?.["l1-3"]?.completed); }
  function progress() {
  ...
  function save(next) {
    ...
    // The existing overview treats a completed L1.5 as unlocked even without L1.4.
    // Revoke that completion too, retaining its checks and all other lesson data.
    if (!next.completed && profile.progress[ID]?.completed && profile.progress["l1-5"]) {
      profile.progress["l1-5"].completed = false;
    }
  ...
  function render() {
    ...
    $("#l14-content").hidden = !open; $("#l14-access").hidden = open;
    $("#l14-access-message").textContent = profile ? "Schließe L1.3 mit allen eigenen Checks und der Lehrkraftbestätigung ab. Danach kannst du hier weiterlernen." : "Lege auf der Startseite dein Lernprofil an und schließe L1.3 ab.";
    ...
    button.textContent = p.completed ? "✓ L1.4 wieder öffnen" : "L1.4 abschließen";
    button.classList.toggle("button-primary", !p.completed); button.classList.toggle("button-secondary", p.completed);
    $("#page-completion-note").textContent = p.completed ? "100 Punkte wurden gutgeschrieben. Beim Wiederöffnen wird L1.5 erneut gesperrt; ein dortiger Abschluss wird ebenfalls zurückgenommen." : "Alle drei eigenen Checks und die Lehrkraftbestätigung sind nötig. Erst der Abschluss schreibt 100 Punkte gut.";
  ...
  document.addEventListener("change", (event) => {
    ...
    if (saved && revoked) toast("Abschluss zurückgenommen. L1.5 ist wieder gesperrt.");
  });
  $("#page-complete-button").addEventListener("click", () => {
    ...
    const saved = save(next); render();
    if (saved) toast(next.completed ? "L1.4 abgeschlossen: 100 Punkte. L1.5 ist freigeschaltet." : "L1.4 ist wieder offen. L1.5 ist wieder gesperrt.");
  });
  $("#next-lesson-link").addEventListener("click", (event) => {
    refresh(); if (!window.EXCEL_LAB_DEV?.enabled && (!unlocked() || !progress().completed)) { event.preventDefault(); render(); toast("Schließe zuerst L1.4 ab."); }
  });
